...
@section('content')
    ...
    <div class="container-fluid new-convoy pt-5">
        ...
        <form method="post" enctype="multipart/form-data" class="mb-5">
            ...
            <div class="row justify-content-center">
                <button class="btn btn-outline-warning btn-sm" type="submit"><i class="fas fa-save"></i> Сохранить конвой</button>
            </div>
            <div class="row">

                <div class="main-info col mt-3">
                    <h3 class="text-primary">Основная информация</h3>
                    @if(!$booking)
                ...
                <div class="communication col mt-3">
                    <h3 class="text-primary">@lang('attributes.communication')</h3>
                ...
                @if(!$booking)
                    <div class="links col mt-3">
                        <h3 class="text-primary">@lang('attributes.links')</h3>
                        ...
                        <div class="form-group">
                            <label for="links[tmp]">Ссылка на мероприятие в TruckersMP</label>
                            <input type="text" class="form-control" id="links[TruckersMP]" name="links[TruckersMP]" value="{{ old('links[TruckersMP]') ?? ($convoy->links['TruckersMP'] ?? '') }}">
                        </div>
                        <div class="form-group">
                            <label for="links[TrucksBook]">Ссылка на мероприятие в TrucksBook</label>
                            <input type="text" class="form-control" id="links[TrucksBook]" name="links[TrucksBook]" value="{{ old('links[TrucksBook]') ?? ($convoy->links['TrucksBook'] ?? '') }}">
                        </div>
                        <div class="form-group">
                            <label for="links[VTCWorld]">Еще ссылка на мероприятие</label>
            ...
            <div class="row">
                <div class="col-4 route-images">
                    <h3 class="text-primary mt-5">@lang('attributes.route')</h3>
                    @if($convoy->start_city)
                        <h6>{{ trans_choice('Текущее изображение|Текущие изображения', $convoy->route) }}</h6>
                        <div class="fotorama mb-3 text-shadow-m" data-allowfullscreen="true" data-nav="thumbs" data-maxheight="300">
                ...
                <div class="col-4">
                    <h3 class="text-primary mt-5">ДЛС</h3>
                    ...
                    <label for="" class="text-muted">Удерживать <i>Ctrl</i> для выбора нескольких или снятия выбора</label>
                    @if($errors->has('dlc'))
            ...
            <div class="row mt-5">
                <div class="truck col">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-danger delete-img mb-2" data-target="truck_image"><i class="fas fa-trash"></i> Удалить картинку</button>
                    @if(!$booking)
                        <div class="custom-control custom-checkbox mb-2">
                ...
                <div class="trailer col">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-danger delete-img mb-2" data-target="trailer_image"><i class="fas fa-trash"></i> Удалить картинку прицепа</button>
                    @if(!$booking)
                        <div class="custom-control custom-checkbox mb-2">
                ...
                <div class="alt_trailer col">
                    ...
                    <div class="form-group alt_trailer_image">
                        ...
                        <img src="/images/convoys/{{ $convoy->alt_trailer_image ? $convoy->alt_trailer_image : "image-placeholder.jpg" }}"
                             class="w-100" id="alt_trailer_image-preview">
                        @if($errors->has('alt_trailer_image'))
                            <small class="form-text">{{ $errors->first('alt_trailer_image') }}</small>
                        @endif
                    </div>
                    <button type="button" class="btn btn-sm btn-outline-danger delete-img mb-2" data-target="alt_trailer_image"><i class="fas fa-trash"></i> Удалить картинку прицепа</button>
                    <div class="form-group">
        ...
        @can('update', \App\Convoy::class)
            @if(count($convoy->audits) > 0)
                <div class="member-changelog container mb-5">
