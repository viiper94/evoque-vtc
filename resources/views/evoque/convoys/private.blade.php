                            ...
                            <div id="convoy-{{ $convoy->id }}-info" class="collapse" aria-labelledby="convoy-{{ $convoy->id }}-header" data-parent="#convoysAccordion-{{ $loop->parent->index }}">
                                <div class="card-body">
                                    @if($convoy->isFulfilled())
                                        ...
                                        @if($convoy->dlc)
                                            <h4 class="pt-5 pb-5 text-center">
                                                <i class="fas fa-exclamation-triangle text-warning"></i>
                                                Для участия требуется
                                                @foreach($convoy->dlc as $item)
                                                    <span style="white-space: nowrap">{{ $item }}@if(!$loop->last), @endif</span>
                                                @endforeach
                                            </h4>
                                        @endif
                                            ...
                                            <section class="col-12 convoy-note py-5 m-auto">
                                                ...
                                                <blockquote class="blockquote text-center my-5">
                                                    ...
                                                    <ol class="text-left ml-5 mr-1 px-md-5">
                                                        ...
                                                        <li>Сборы проходят на нашем Дискорд-сервере и только за 10 минут до старта
                                                            переходим на сервер организаторов конвоя или остаёмся на нашем если это закрытый конвой.</li>
