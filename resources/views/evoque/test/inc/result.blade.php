<div class="row my-5 py-5 flex-column align-items-center">
    @if($count > 0)
        ...
        <p class="text-muted">Через {{ $first_answered->created_at->addDays(31)->diffInDays(\Carbon\Carbon::now()) }}
            {{ trans_choice('день|дня|дней', $first_answered->created_at->addDays(31)->diffInDays(\Carbon\Carbon::now())) }} ваши результаты будут обнулены.</p>
